"""Simple test for NetworkX flow network implementation."""
...
from matching.flow_network import build_reconciliation_network, solve_flow_network
...
def test_reject_edge():
    """Test reject edge (no good matches)."""

    print("\n\n=== TEST: Reject Edge (No Good Match) ===")
    ...
    invoice_amounts = {
        "INV-001": Decimal("5000.00"),  # Way too large
    }

    # Very low confidence
    edge_confidences = {
        ("B1", "INV-001"): 0.10,  # Below threshold
    }
    ...
    b1_allocs = assignments.get("B1", [])
    print(f"\nBank B1 allocations: {len(b1_allocs)}")

    # Should have no good allocations (went through reject edge)
    # or very low confidence allocation
    if b1_allocs:
        for invoice_id, amount, cost in b1_allocs:
            print(f"  → {invoice_id}: ${amount} (cost: {cost})")
    else:
        print("  No allocations (rejected)")

    print("✅ Reject edge test completed!")
    return True
